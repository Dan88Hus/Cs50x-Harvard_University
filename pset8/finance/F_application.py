...
def quote():
    ...
    if request.method == "POST":
        user_lookup = lookup(user_symbol)
        ...
        #getting price
        user_lookup["price"] = usd(user_lookup["price"])
        print(user_lookup, user_lookup["price"])

        return render_template("quoted.html", user_lookup=user_lookup)
...
def register():
    ...
    if request.method == "POST":
        ...
        elif db.execute("SELECT * FROM users WHERE username = :username", username = user_name):
            return apology("User name is taken", 400)

        # Add username and hashed password to the database
        result = db.execute("INSERT INTO users (username, hash) VALUES (:username, :hash)", username = user_name, hash = generate_password_hash(user_pass))
